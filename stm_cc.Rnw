...
\hh{You're sneaking in the intense resource requirements through the backdoor.}
...
\hh{something went wrong in the next sentence}
An early example of the efforts to apply statistically sound methods to resonably model the latent processes that could have generated the documents in a corpus is Probabalistic Latent Semantic Indexing (pLSI) \citep{hofmann:1999}. \hh{XXX bit more detials: what is the main idea behind pLSI} Despite many limitations, pLSI was a pioneering step in the development of the foundation for the Topic Models framework.
...
\jp{I'm still need to do a lot with this part, but I wanted to push what I had to the repo so I could get feedback at least on that part.} \hh{:)}
