...
``Simple" Latent Dirichlet Allocation is the base form of this method, built on the common assumptions of exchangeability and independence. \hh{define exchangeability and independence in the context of LDA} Unsupervised classification problem in machine learning (topics are not known before hand)

Unlike previous probabilistic methods to model documents, such as probabilistic latent semantic indexing (hoffman 99), LDA is a (vast/stark?) improvement because it can assign probabilities beyond the documents included when the model was fit to documents not originally included. 
%\hh{ give an example (with citation) of a previous method}

A major limitation of LDA is its inability to incorporate correlation across topics. 
In practice, independence between documents is an unrealistic expectation when dealing with large sets of documents.
...
For example, in the ANES dataset, document level covariates would include the respondent's gender, race, and/or party affiliation.
