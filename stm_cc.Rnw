...
From here, the STM package (or LDA package) can be applied to the data, and topics and their proportions in the documents and the corpus can be estimated and displayed. In Figure \ref{stm-plot}, we see the topics, ordered from highest to lowest proportion. The three highest occurence words are included with each topic. Topic numbers are not in any particular order.
