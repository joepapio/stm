...
#consider changing sparse level to control words that show up infrequently
#xkcd <- textProcessor(metadata = dfTranscripts, documents=dfTranscripts$text, sparselevel=1)

# step by step text processing using the tm package
dfTranscripts$Ptext <- dfTranscripts$text
#strip whitespace
dfTranscripts$Ptext <- tm_map(dfTranscripts$Ptext, stripWhitespace)
